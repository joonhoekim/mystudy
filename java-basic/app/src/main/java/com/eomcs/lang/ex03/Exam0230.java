...
/*
 * # 메모리 크기와 정수의 범위 1 비트: 2**1 - 음수가 없는 경우: 0, 1 2 비트: 2**2 - 음수가 없는 경우: 00(0), 01(1), 10(2), 11(3)
 * (0 ~ 3) - 음수가 있는 경우: 00(0), 01(1), 10(-2), 11(-1) (-2 ~ 1) 4 비트: 2**4 - 음수가 없는 경우: 0 ~ 2**4 - 1 =
 * 0 ~ 15 - 음수가 있는 경우: -2**3 ~ 2**3 - 1 = -8 ~ 7 8 비트: 2**8 - 음수가 없는 경우: 2**8 = 0 ~ 2**8-1 = 0 ~ 255
 * - 음수가 있는 경우: -2**7 ~ 2**7-1 = -128 ~ 127 16비트: 2**16 - 음수가 없는 경우: 0 ~ 2**16-1 = 0 ~ 65535 - 음수가
 * 있는 경우: -2**15 ~ 2**15-1 = -32768 ~ 32767 32비티: 2^32 - 음수가 없는 경우: 0 ~ 2**32 - 1 = 4,294,967,296 -
 * 음수가 있는 경우: -2**31 ~2**31 - 1 = -2,147,483,648 ~ 2,147,483,647 64비트: 2^64 = - 음수가 있는 경우: 약 -900경 ~
 * 900경
 *
 * ## 정리 - 4바이트 정수 리터럴 예) 100
 *
 * - 8바이트 정수 리터럴 정수 뒤에 접미사 L 또는 l을 붙인다. 예) 100L (주로 사용) 예) 100l
 */
